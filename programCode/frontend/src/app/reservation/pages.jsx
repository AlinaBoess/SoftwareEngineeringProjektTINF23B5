...
function MainComponent() {
    async function checkAuth() {
        try {
            const res = await fetch("https://deine-domain.de/api/user", {
                credentials: "include", // wichtig für Cookie-Support
            });
            if (res.ok) {
                const data = await res.json();
                setUser(data); // z. B. { name: "Anna" }
            }
        } catch (err) {
  ...
  const [user, setUser] = useState(null); // example: { name: "Anna" }  
    ...
    <div className="bg-white p-8 rounded-lg max-w-sm w-full">
      ...
      <form
        onSubmit={(e) => {
          ...
          // Hier könntest du auch API-Call machen
          if (authMode === "register" && !name) return alert("Bitte Name angeben");
          setUser({ name: authMode === "login" ? "Gast" : name }); // Fake-Login
